# 京介｜フリーランス Web Support ポートフォリオ

WordPressの修正・保守、サーバー・DNS設定、Google Tag Managerの導入、Contact Form 7のカスタマイズを紹介する、**1ページ完結の静的ポートフォリオサイト**です。HTML、CSS、JavaScriptのみで動作し、ビルド作業を行わずにGitHub Pagesへ公開できます。

> **公開前の重要事項**  
> お問い合わせボタンは現在、ランサーズとココナラのトップページへ接続しています。公開前に、`index.html` 内のURLを京介さん自身のプロフィールURLへ差し替えてください。
